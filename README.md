...
Practice kit for Anthropic's Claude Certified Architect: Foundations exam. Content built from the official exam guide PDF + Hamza Farooq's community repo (github.com/hamzafarooq/claude-certified-architect).
...
Tool descriptions as the routing mechanism — minimal descriptions break everything. `isError: false` for zero results vs `isError: true` for actual failures. `tool_choice: "any"` vs `"auto"`. Scoped tool access (4–5 per agent, not 18). `.mcp.json` project-level vs `~/.claude.json` personal.
...
When a single failure causes financial loss, a security breach, or a compliance violation — hooks. Always. Doesn't matter how specific or well-worded the prompt is. The exam tests this distinction relentlessly across all 5 domains.
